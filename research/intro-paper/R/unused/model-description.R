...
# get forecasts
source(here(subproj_dir, "R", "get-model-forecasts.R"))

# get participating team's institutional locations
team_countries <- read_csv(here("research", "thematic-eval", "eval-by-country", 
                                "team-country-institution.csv"))
...
# Heatmap: 1-wk ahead forecasts over time ------------------------------------
forecasts_summary <- forecasts_ex_hub %>%
  filter(horizon == 1) %>%
  group_by(forecast_date, location_name, target_variable) %>%
  summarise(n_predictions = n())

# plot number of forecast predictions
forecasts_summary %>%
  ggplot(aes(x = forecast_date, y = location_name, 
             fill = n_predictions)) +
  geom_tile() +
  labs(x = NULL, y = NULL,
       fill = "Number of one week ahead predictions") +
...
ggsave(height = 5, width = 5,
       filename = paste0(subproj_dir, "/figures/", "model-predictions-heatmap.png"))

# plot number of models
forecasts_summary_models <- forecasts_ex_hub %>%
  filter(horizon == 1 &
           type == "point") %>%
  group_by(forecast_date, location_name, target_variable) %>%
  summarise(n_models = n())

forecasts_summary_models %>%
  ggplot(aes(x = forecast_date, y = location_name, 
             fill = n_models)) +
  geom_tile() +
  labs(x = NULL, y = NULL,
       fill = "Number of models contributing \n 1 week ahead point forecasts") +
  facet_wrap(~ target_variable) +
  scale_fill_distiller(direction = 1) +
  scale_x_date(date_breaks = "6 week", date_labels = "%b") +
  theme_bw() +
