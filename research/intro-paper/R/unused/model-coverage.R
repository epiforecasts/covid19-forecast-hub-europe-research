...
here::i_am(paste0(subproj_dir, "/R/model-coverage.R"))
# packages
library(forcats)
library(ggplot2)
library(dplyr)
...
# Data --------------------------------------------------------------------
# Keep only the 2 week horizon
cov <- eval_wide %>%
  filter(horizon == 2) %>%
  select(model, target_variable, location, horizon, weeks_included,
         cov_50, cov_95)
...
cov %>%
  ...
  labs(y = NULL, 
       ...
       caption = paste("Evaluation as of", eval_date)) +
